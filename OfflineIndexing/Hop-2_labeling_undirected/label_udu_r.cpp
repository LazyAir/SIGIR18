...
void sumDeg(char * binName, char * degName)
{
	outBuf degBuf(degName);
	...
	while (!edgeBuf.isEnd)
	{
		...
		{
			degBuf.insert(xx, cnt, 0);
			for (int i = xx+1; i < tmp.x; i++)
				degBuf.insert(i, 0, 0);
...
void init(char * binName, char *oldOutName)
{
	inBuf edgeBuf(binName);

	outBuf oldOutBuf(oldOutName);
...
double checkB(char * fileName)
{
	FILE * pFile = fopen (fileName,"rb");
	fseek (pFile, 0, SEEK_END); 
	double ans =ftell(pFile)+0.0;
	fclose(pFile);
	return ans;
}
...
//				if (iter == 6 && ix >= 1200 && ix <= 1300) printf("%d %d =\n", ix, mem[tid].x);
...
//				for (int i = stid, di = -1; i < perMem && mem[i].x == ix; i += di)
				{

//				if (iter == 6 && ix >= 1200 && ix <= 1300) printf("%d\n", i);
...
//				if (iter == 6 && ix >= 1240 && ix <= 1250) printf("%d %d %d ====\n", ix, mem[j].y, mem[j].w + mem[i].w);
							
//							if (ix == 1247 && mem[j].y == 1116 && mem[j].w+mem[i].w == 2) printf("%s %d %d +++\n", outAnsName, mem[j].x, mem[j].w);
...
//				if (iter == 6 && ix >= 1240 && ix <= 1250) printf("%d %d %d %d %d====\n", ix, mem[j].y, mem[j].w + mem[i].w, xxid, yyid);
...
//					if (i <= stid)if (i == ptail || mem[i-1].x != ix){	i = stid;						di = 1;						}
...
void prune(char * dataName, char * scanName, char * allName, char * newName, bool & isEnd, int & maxDeg)
{	
	
	printf("Prune can scan %lld %lld\n", checkSize(dataName), checkSize(scanName) );
	
	inBuf dataBuf(dataName), scanBuf(scanName), oldBuf(scanName);
...
//void prune(char * dataName, char * scanName, char * allName, char * newName, bool & isEnd)


void pruneSmall(char * canName, char * scanName, char * allName, char * newName, bool & isEnd, int & maxDeg)
{
	
	printf("PruneSmall can scan %lld %lld\n", checkSize(canName), checkSize(scanName) );
	
	inBuf canEdge(canName), oldEdge(scanName), scanEdge(scanName) ;
...
void appendInOut(char * degName, char * degInName, char * degOutName, char * ansName)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		ansBuf.insert( ((((long long)e.y)<<32) | e.x), d1.x, d2.x);
...
void run()
{
	...
	init(binName, oldOutName);
	
	xSort(oldOutName, m, false);
	
	copyFile(oldOutName, gOutName);
	copyFile(oldOutName, newOutName);

	int maxDeg = checkMaxDeg(oldOutName);
...
printf("newOut oldOut\n%lld %lld\n", checkSize(newOutName), checkSize(oldOutName));
...
tRun.restart();
		...
		if (iter <= 10)
		{
			...
			genNew(newOutName, gOutName, canOutName, isEnd);
		}
		else
		{
			copyFile(oldOutName, canOutName);
			genNew(newOutName, oldOutName, canOutName, isEnd);
		}
...
{			long long tmpSize = checkSize(canOutName);			if (largeDisk < tmpSize) largeDisk = tmpSize;}
...
if (fmerge) printf("prune out begin\n");	
		...
		if (checkSize(canOutName) > checkSize(oldOutName)) 
			prune(canOutName, oldOutName, oldOutTmpName, newOutName, isEnd, maxDeg);
		else pruneSmall(canOutName, oldOutName, oldOutTmpName, newOutName, isEnd, maxDeg);
...
if (findex)	printf("index begin\n");
	
	char * outName = (char *)malloc(1+strlen(txtName) + 4);	
	...
	long long nLL = n;
	swapEdge(degName);
	xSort(degName, nLL, false);
	appendInOut(degName, degOutName, degOutName, tmpDegName);		
	xSortL(tmpDegName, nLL);
	remove(degName); rename(tmpDegName, degName);
	
if	(fdeb) printf("sumCnt indexSize %lld %lf MB\n", sumCnt, (checkB(outName)+checkB(degName))/(1024*1024) );
